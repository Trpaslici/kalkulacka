...
double MaafLajbrry::add(double a, double b) { // secte 2 cisla

	return (a + b);
}
//----------------------------------------------------
double MaafLajbrry::sub(double a, double b) { // odecte 2 cisla
	
	return (a - b);
}
//----------------------------------------------------
double MaafLajbrry::mul(double a, double b) { // vynasobi 2 cisla

	return (a * b);
}
//----------------------------------------------------
double MaafLajbrry::divid(double a, double b) { // vydeli 2 cisla

	if (b == 0) {
		fprintf(stderr, "Error, deleni nulou\n");
		throw new std::runtime_error("Zadan nulovy delitel\n");
	}
	else {
		return (a / b);
	}
...
//----------------------------------------------------
long long MaafLajbrry::factorial(int a) { // vrati faktorial celeho kladneho cisla

	long long pom = 1;
	if (a < 0) {
		fprintf(stderr, "Error, zadejte kladne cele cislo\n");
		throw new std::runtime_error("Zadano zaporne cislo!\n");
	}
	else if (a > 20) {
		fprintf(stderr, "Error, zadejte kladne cele cislo\n");
		throw new std::runtime_error("Zadano prilis velke cislo!\n");
	}
...
//----------------------------------------------------
double MaafLajbrry::rad(double a, double b) { // vypocita odmocninu b|/a
	double Min = 1;
	double Max;
	double pom;
	double diff;
	...
	if ((((long long)b % 2) == 0) && (a < 0)) {      //neni lichou odmocninou a pod odmocninou je zaporne cislo
		...
		throw new std::runtime_error("Zadano zaporne cislo pod sudou odmocninou!\n");
	}
...
//----------------------------------------------------
long long MaafLajbrry::komb (int a, int b) // vypocita kombinacni cislo a nad b
{	
	long long citatel = 1;
	int opti_b = b;
	int max_a = 0;
	...
	{
		fprintf(stderr, "Math ERROR \n");
		throw new std::runtime_error("Zadano k vetsi jak n\n");
	}
	...
	else // jinak vypocitej jako: a nad b = (a*(a-1)*(a-2)*...*(a-b+1)) / factorial(b)
	{
		...
		{
			fprintf(stderr, "Math ERROR \n");
			throw new std::runtime_error("Zadano prilis velke k");
		}
		...
		{
			fprintf(stderr, "Math ERROR \n");
			throw new std::runtime_error("N mimo rozsah pro dane K\n");
		}
...
//----------------------------------------------------
double MaafLajbrry::invert(double a) // vrati cislo s opacnym znamenkem
{
	if (a == 0)
		return 0;
...
//----------------------------------------------------
int MaafLajbrry::checkInt(double a) // zkontroluje, jestli je v double a ulozeno cele cislo
{
	if (a - (long long)a == 0)
		return 1;
...
//----------------------------------------------------
long long MaafLajbrry::doubleToInt(double a) // prevede double na long long int
{
	return (long long)a;
}
